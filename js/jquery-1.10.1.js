 ...
 *
 *日期：2013-05-30T21：49Z
 * /
（函数（窗口，未定义）{

//无法执行此操作，因为包括ASP.NET在内的多个应用程序都在跟踪
...
变种
	...
	//支持：IE <10
	//对于`typeof xmlNode.method`而不是`xmlNode.method！== undefined`
	core_strundefined =未定义的typeof，
	...
	//确保我们裁剪了BOM和NBSP（在这里，您是Safari 5.0和IE）
	...
	已完成=函数（事件）{
		...
		// readyState ===“ complete”足以让我们在oldIE中调用dom ready
		如果（document.addEventListener || event.type ===“ load” || document.readyState ===“ complete”）{
			分离（）;
			jQuery.ready（）;
		}
	}，
	...
	detach = function（）{
		如果（document.addEventListener）{
			...
			document.detachEvent（“ onreadystatechange”，完成）;
			window.detachEvent（“ onload”，完成）;
		}
	};
...
jQuery.extend（{
	...
	准备好了：function（wait）{
		...
		//如果有待处理的保全或我们已经准备就绪，则中止
		if（wait === true？--jQuery.readyWait：jQuery.isReady）{
			返回;
		}

		//至少要确保主体存在，以防IE有点过分热情（票号＃5443）。
		如果（！document.body）{
	...
	isNumeric：function（obj）{
		返回！isNaN（parseFloat（obj））&& isFinite（obj）;
	}，
	...
	isPlainObject：function（obj）{
		...
		//支持：IE <9
		//在自己的属性之前处理继承属性的迭代。
		如果（jQuery.support.ownLast）{
			for（key in obj）{
				返回core_hasOwn.call（obj，key）;
			}
		}
		...
		返回键===未定义|| core_hasOwn.call（obj，key）;
	}，
	...
	//数据：HTML字符串
	//上下文（可选）：如果指定，将在此上下文中创建片段，默认为document
	// keepScripts（可选）：如果为true，将包括在html字符串中传递的脚本
	parseHTML：function（数据，上下文，keepScripts）{
		...
		//单个标签
		如果（已解析）{
			返回[context.createElement（parsed [1]）];
		}

		解析= jQuery.buildFragment（[data]，context，scripts）;
		如果（脚本）{
			jQuery（脚本）.remove（）;
		}
	...
	parseJSON：function（data）{
		//首先尝试使用本机JSON解析器进行解析
		如果（window.JSON && window.JSON.parse）{
	...
	修剪：core_trim &&！core_trim.call（“ \ uFEFF \ xA0”）吗？
		功能（文字）{
			返回文字== null？
				“”：
				core_trim.call（text）;
	...
	//用来获取和设置集合值的多功能方法
	//如果值是一个函数，则可以选择执行
	访问：function（elems，fn，key，value，chainable，emptyGet，raw）{
		var i = 0，
			长度= elems.length，
			批量=键==空;

		//设置许多值
		如果（jQuery.type（key）===“对象”）{
			chainable = true;
			为（我在键中）{
	...
	//一种快速换入/换出CSS属性以获得正确计算的方法。
	//注意：该方法属于css模块，但在这里对于support模块是必需的。
...
jQuery.ready.promise = function（obj）{
	如果（！readyList）{
		...
		//捕获发生浏览器事件后调用$（document）.ready（）的情况。
		//我们曾经尝试在此处使用readyState“ interactive”，但这引起了类似的问题
		...
		}否则，如果（document.addEventListener）{
			//使用方便的事件回调
		...
		//如果使用IE事件模型
			...
			//确保在加载前触发，可能要晚触发，但对于iframe也安全
			document.attachEvent（“ onreadystatechange”，已完成）;

			//回退到window.onload，它将始终有效
			window.attachEvent（“ onload”，完成）;
...
函数isArraylike（obj）{
	...
	返回类型===“数组” || 输入！==“ function” &&
		（长度=== 0 ||
		typeof length ===“数字” &&长度> 0 &&（obj中的length-1））;
}
...
var i，
	...
	//可接受的运算符http://www.w3.org/TR/selectors/#attribute-selectors
	...
	pseudos =“ :(” + characterEncoding +“）（？：\\（（（['\”]）（（？：\\\\。| [^ \\\\]）*？）\\ 3 | （（?? \\\\。| [^ \\\\（）[\\]] |“ + attribute.replace（3，8）+”）*）|。*）\\）|）“，
